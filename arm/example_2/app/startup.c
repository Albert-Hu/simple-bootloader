...
extern void main(struct boot_config *config);
...
void reset_handler(uint32_t config_addr)
{
	...
	/*
		Copy the config from the bootloader.
		NOTE: The config is a global variable stored in the BSS segment.
	*/
	memcpy(&config, (void*) config_addr, sizeof(struct boot_config));
